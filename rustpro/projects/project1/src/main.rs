...
fn main() {
    println!("Hello, this is project related to ownership!");

    let is_concert: bool = true;
    let is_event = is_concert;

    println!("{is_concert} and {is_event}");

    let sushi = "salmon";
    let dinner = sushi;

    println!("{dinner} and {dinner}");

    let meals = String::from("Sambhar rice thali");
    let meals = eat_meal(meals);
    println!("{meals}");
}
